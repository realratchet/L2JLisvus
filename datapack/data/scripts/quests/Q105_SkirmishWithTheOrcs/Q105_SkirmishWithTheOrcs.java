...
public class Q105_SkirmishWithTheOrcs extends Quest
{
	...
	{
		super(105, Q105_SkirmishWithTheOrcs.class.getSimpleName(), "Skirmish with the Orcs");
		...
		switch (st.getState())
		{
			case State.CREATED:
				if (player.getRace() != Race.ELF)
					htmltext = "7218-01.htm";
				else if (player.getLevel() < 10)
					htmltext = "7218-09.htm";
				else
	...
	public String onKill(L2NpcInstance npc, L2PcInstance killer, boolean isPet)
	{
		...
		switch (npc.getNpcId())
		{
			...
			case KABOO_CHIEF_TANUKIA:
				if (st.getInt("cond") == 1 && st.hasQuestItems(npc.getNpcId() - 3223)) // npcId - 3223 = itemId to verify.
				{
					st.set("cond", "2");
					st.playSound(QuestState.SOUND_MIDDLE);
			...
			case KABOO_CHIEF_ROKO:
				if (st.getInt("cond") == 3 && st.hasQuestItems(npc.getNpcId() - 3224)) // npcId - 3224 = itemId to verify.
				{
					st.set("cond", "4");
					st.playSound(QuestState.SOUND_MIDDLE);
			...
			case KABOO_CHIEF_MURTIKA:
				if (st.getInt("cond") == 3 && st.hasQuestItems(npc.getNpcId() - 3225)) // npcId - 3225 = itemId to verify.
				{
					st.set("cond", "4");
					st.playSound(QuestState.SOUND_MIDDLE);
					st.giveItems(KABOO_CHIEF_TORC_2, 1);
