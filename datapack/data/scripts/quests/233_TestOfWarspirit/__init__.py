...
class Quest (JQuest) :
	...
	def onTalk(self,npc,st):
		...
		# if quest is already completed
			...
			return JQuest.getAlreadyCompletedMsg()
...
#		return the current value of the var
		...
		if st.getInt(var) in value and random < chance:
			...
			if npcId in [89,90]:
				if not (st.getQuestItemsCount(KIRUNAS_THIGH_BONE_ID) and st.getQuestItemsCount(KIRUNAS_ARM_BONE_ID)) :
					list = 0
					chance = 70
				elif not (st.getQuestItemsCount(KIRUNAS_SPINE_ID) and st.getQuestItemsCount(KIRUNAS_RIB_BONE_ID)) :
					list = 1
			 		chance = 40
			 	elif not st.getQuestItemsCount(KIRUNAS_SKULL_ID) :
			 		list = 2
			 		chance = 10
			 	else :
		  			return
		  		if random > chance :
			   	...
			   	for item in itemList[list]:
					...
					if st.getQuestItemsCount(KIRUNAS_SKULL_ID) and st.getInt(var) == 9:
						st.set(var,"10")
			...
			# Drop part for any other mobs
				...
				for item in itemList:
					...
					if count < maxcount:
						amount = 1
						if item in [EXCUROS_SCALE_ID,MORDEOS_TALON_ID]: amount = 5
						if item == [PORTAS_EYE_ID]: amount = 2
