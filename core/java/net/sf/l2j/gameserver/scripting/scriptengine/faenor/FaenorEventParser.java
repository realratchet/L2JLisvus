...
import net.sf.l2j.gameserver.scripting.scriptengine.IntList;
import net.sf.l2j.gameserver.scripting.scriptengine.Parser;
import net.sf.l2j.gameserver.scripting.scriptengine.ParserFactory;
...
{
    private static Logger _log = Logger.getLogger(FaenorEventParser.class.getName());
    ...
    private void parseEventMessage(Node sysMsg)
    {
        ...
        try
        {
            ...
            String[] messages = attribute(sysMsg, "Msg").split("\n");
            _eventDates.setMessages(messages);
            
            if (type.equalsIgnoreCase("OnJoin"))
            {
                bridge.onPlayerLogin(_eventDates);
            }
        } 
        catch (Exception e)
        {
    ...
    private void parseEventDrop(Node drop)
    {
        ...
        try
        {
            ...
            bridge.addEventDrop(items, count, chance, _eventDates);
        }
        catch (Exception e)
        {
