...
final class DocumentSkill extends DocumentBase
{
	...
	protected void parseSkill(Node n)
	{
		...
		{
			throw new RuntimeException("Skill id=" + skillId + " number of levels mismatch, " + lastLvl + " levels expected");
		}
		...
		for (int i = 0; i < enchantLevels1; i++)
		{
			...
			for (n = first; n != null; n = n.getNextSibling())
			{
				...
				else if ("enchant1".equalsIgnoreCase(n.getNodeName()))
				{
		...
		for (int i = 0; i < enchantLevels2; i++)
		{
			...
			for (n = first; n != null; n = n.getNextSibling())
			{
				...
				else if ("enchant2".equalsIgnoreCase(n.getNodeName()))
				{
