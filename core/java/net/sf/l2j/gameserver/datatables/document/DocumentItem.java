...
final class DocumentItem extends DocumentBase
{
			...
			if ("list".equalsIgnoreCase(n.getNodeName()))
			{
				for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
				{
					if ("item".equalsIgnoreCase(d.getNodeName()))
					{
						...
						{
							_log.warning(getClass().getSimpleName() + ": Cannot create item " + _currentItem.id + ": " + e);
						}
					}
				}
			}
	...
	protected void parseItem(Node n) throws InvocationTargetException
	{
		...
		// At this point, item hasn't necessarily been created yet
		makeItem();
	}
	...
	private void makeItem() throws InvocationTargetException
	{
		...
		try {
			Constructor<?> c = Class.forName("net.sf.l2j.gameserver.templates.L2" + _currentItem.type).getConstructor(StatsSet.class);
