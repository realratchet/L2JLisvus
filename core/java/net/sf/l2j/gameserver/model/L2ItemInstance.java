...
public final class L2ItemInstance extends L2Object
{
	...
	public boolean isAvailable(L2PcInstance player, boolean allowAdena, boolean allowEquipped)
	{
		return ((allowEquipped || !isEquipped()) // Not equipped (conditionally)
			...
			&& (allowAdena || _itemId != Inventory.ADENA_ID) && (player.getCurrentSkill() == null || player.getCurrentSkill().getSkill().getItemConsumeId() != _itemId) && (isTradable()));
	}
			...
			PreparedStatement statement = con.prepareStatement("SELECT owner_id, object_id, item_id, count, enchant_level, loc, loc_data, price_sell, price_buy, custom_type1, custom_type2 FROM items WHERE object_id = ?"))
		{
			...
			try (ResultSet rs = statement.executeQuery())
			{
				if (rs.next())
				{
					...
					try
					{
						item = new L2ItemInstance(objectId, itemId);
						item._ownerId = ownerId;
						item._count = count > getMaxItemCount(itemId) ? getMaxItemCount(itemId) : count;
						item._enchantLevel = enchantLevel;
						item._type1 = customType1;
						item._type2 = customType2;
						item._loc = loc;
						item._locData = locData;
						item._priceSell = priceSell;
						item._priceBuy = priceBuy;
						item._existsInDb = true;
						item._storedInDb = true;
					}
					catch (Exception e)
					{
						_log.log(Level.SEVERE, "Could not find item template for ID '" + itemId + "': ", e);
					}
				}
				else
				{
