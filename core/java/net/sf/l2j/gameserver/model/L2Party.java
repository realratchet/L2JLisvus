...
{
	private final double[] _bonusExpSp =
	{
		1,
		1.30,
		1.39,
	...
	public void distributeItem(L2PcInstance player, L2Attackable.RewardItem item, boolean spoil, L2Attackable target)
	{
		...
		if (item.getCount() > 1)
		{
			SystemMessage msg = spoil ? new SystemMessage(SystemMessage.S1_SWEEPED_UP_S2_S3) : new SystemMessage(SystemMessage.S1_PICKED_UP_S2_S3);
		...
		{
			SystemMessage msg = spoil ? new SystemMessage(SystemMessage.S1_SWEEPED_UP_S2) : new SystemMessage(SystemMessage.S1_PICKED_UP_S2);
			msg.addString(looter.getName());
			msg.addItemName(item.getItemId());
			broadcastToPartyMembers(looter, msg);
		}
	...
	public void distributeAdena(L2PcInstance player, int adena, L2Character target)
	{
		...
		List<L2PcInstance> memberList = getPartyMembers();
		
		// Check the number of party members that must be rewarded
		// (The party member must be in range to receive its reward)
		List<L2PcInstance> toReward = new ArrayList<>();
		
		for (L2PcInstance member : memberList)
		{
